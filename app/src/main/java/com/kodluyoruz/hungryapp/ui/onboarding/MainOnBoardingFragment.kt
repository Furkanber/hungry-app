...
class MainOnBoardingFragment : BaseFragment() {
    ...
    /*private fun initIndicators() {
        for (i in 0 until fragmentList.size) {
            ...
            when (i) {
                0 -> {
                    indicator.background = ResourcesCompat.getDrawable(
                        resources,
                        R.drawable.viewpager_indicator_active,
                        null
                    )
                ...
                indicator.background = ResourcesCompat.getDrawable(
                    resources,
                    R.drawable.viewpager_indicator_inactive,
                    null
                )
    ...
    private fun handlePageChange(position: Int) {
        for (i in 0 until fragmentList.size) {
            if (i == position) {
                ...
                binding.viewPagerIndicator.getChildAt(i).background = ResourcesCompat.getDrawable(
                    resources,
                    R.drawable.viewpager_indicator_active,
                    null
                )
