...
class EditProfileFragment : BaseFragment() {
    ...
    private fun initViews() {
        ...
        binding.buttonChangePasswordNavigation.setOnClickListener {
            findNavController().navigate(
                EditProfileFragmentDirections.actionEditProfileFragment2ToChangePasswordFragment(
                    user
                )
            )
        }
    ...
    private fun updateProfile() {
        ...
        viewModel.updateUser(token.toString(), user)
            .observe(viewLifecycleOwner, object : Observer<Resource<UsersItem>> {
